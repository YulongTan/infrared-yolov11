...
class Head(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        box = torch.cat(tensors=((a + b) / 2 / 640, (b - a) / 640), dim=1)  # 除以640进行归一化
...
def yolo_v11_n(num_classes: int = 80):
    csp = [False, True]
    depth = [1, 1, 1, 1, 1, 1]
    # width = [3, 16, 32, 64, 128, 256]#visible
    width = [4, 16, 32, 64, 128, 256]  #infrared
...
def yolo_v11_s(num_classes: int = 80):
    csp = [False, True]
    depth = [1, 1, 1, 1, 1, 1]
    width = [3, 32, 64, 128, 256, 512]#visible
    # width = [4, 32, 64, 128, 256, 512]#RGBA
    return YOLO(width, depth, csp, num_classes)
