...
data_dir = 'D:\\pycharm_proj\\yolov11\\LLVIP'


def train(args, params):
    # Model
    model = nn.yolo_v11_n(len(params['names']))
    # model = torch.load(f='./weights/best.pt', weights_only=False, map_location='cuda')
    ...
    with open(f'{data_dir}\\train.txt') as reader:
        for filename in reader.readlines():
            ...
            filenames.append(f'{data_dir}\\train\\RGBT\\' + filename)
    ...
    #     for filename in reader.readlines():
    #         filename = os.path.basename(filename.rstrip())
    #         filenames.append(f'{data_dir}\\train\\visible\\' + filename)  # 可见光图像
    #         filenames_ir.append(f'{data_dir}\\train\\infrared\\' + filename)  # 红外图像
    #
    # sampler = None
    # dataset = Dataset(filenames, filenames_ir, args.input_size, params, augment=True)
    #
    # #END infrared
...
def test(args, params, model=None):
    #visible
    filenames = []
    with open(f'{data_dir}\\validate.txt') as reader:
        for filename in reader.readlines():
            filename = os.path.basename(filename.rstrip())
            filenames.append(f'{data_dir}\\test\\RGBT\\' + filename)
            # filenames.append(f'{data_dir}\\test\\RGBA\\' + filename)
    ...
    #     for filename in reader.readlines():
    #         filename = os.path.basename(filename.rstrip())
    #         filenames.append(f'{data_dir}\\test\\visible\\' + filename)  # 可见光图像
    #         filenames_ir.append(f'{data_dir}\\test\\infrared\\' + filename)  # 红外图像
    #
    # dataset = Dataset(filenames, filenames_ir, args.input_size, params, augment=False)
    #
    # #END infrared

    loader = data.DataLoader(dataset, batch_size=32, shuffle=False, num_workers=8,
                             pin_memory=True, collate_fn=Dataset.collate_fn)

    if model is None:
        model = torch.load(f='./weights/best_ap66.pt', weights_only=False, map_location='cuda')
...
def profile(args, params):
    import thop
    # shape = (1, 3, args.input_size, args.input_size)#visible
    shape = (1, 4, args.input_size, args.input_size)#infrared
