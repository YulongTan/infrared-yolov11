...
# 将绝对路径添加到 sys.path
sys.path.append("D:\\yolov11\\YOLOv11-pt-master\\nets")
from nets import nn
...
def train(args, params):
    ...
    model.fuse()
    # Step 2: 准备模型用于QAT（量化感知训练）
    model.qconfig = torch.quantization.get_default_qat_qconfig('fbgemm')
    torch.quantization.prepare_qat(model, inplace=True)
    model = model.to(torch.float32) # 确保模型的每个子模块为 float32
    ###################################################
...
def test(args, params, model=None):
    filenames = []
    with open(f'D:/yolov11/YOLOv11-pt-master/Dataset/COCO/val2017.txt') as reader:
        for filename in reader.readlines():
            filename = os.path.basename(filename.rstrip())
    ...
    for samples, targets in p_bar:
        ...
        # Filter: Only keep detections for "person" (id=1)
        person_class_idx = 1  # Person class id is typically 1 for COCO
        filtered_outputs = []
        for output in outputs:
            # Filter out detections that are not class "person"
            output = output[output[:, 5] == person_class_idx]
