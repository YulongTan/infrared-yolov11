...
class Dataset(data.Dataset):
    ...
    # #infrared
    # def __init__(self, filenames, filenames_ir, input_size, params, augment):
    # #END infrared
        self.params = params
        self.mosaic = augment
        ...
        # #infrared
        # self.filenames = filenames  # 可见光图路径
        # self.filenames_ir = filenames_ir  # 红外图路径
        # #END infrared
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            # Albumentations
            # image, box, cls = self.albumentations(image, box, cls)#visible
            #RGBA
            if image.shape[2] == 4:
                rgb_image = image[:, :, :3]
                alpha_channel = image[:, :, 3:]

                rgb_image, box, cls = self.albumentations(rgb_image, box, cls)
                image = numpy.concatenate([rgb_image, alpha_channel], axis=2)
            # # infrared
            # if image.shape[2] == 6:
            #     ir_rgb = image[:, :, :3]
            #     vis_rgb = image[:, :, 3:]
            #
            #     # 只对可见光做增强（或者对两个都做增强）
            #     vis_rgb, box, cls = self.albumentations(vis_rgb, box, cls)
            #     ir_rgb, _, _ = self.albumentations(ir_rgb, box, cls)  # 可选：对红外也增强
            #
            #     # 拼接回6通道
            #     image = numpy.concatenate([ir_rgb, vis_rgb], axis=2)
            # #END infrared
            else:
            ...
            # #infrared
            # # image: [H, W, 6]，前3通道是红外，后3通道是可见光
            # image_ir = image[:, :, :3]
            # image_vis = image[:, :, 3:]
            #
            # # 增强可见光图像
            # augment_hsv(image_vis, self.params)
            #
            # # 合并为新的6通道图像
            # image = numpy.concatenate([image_ir, image_vis], axis=2)
            # #END infrared
        ...
        # sample = image.transpose((2, 0, 1))[::-1]#visible
        #RGBA
        if image.shape[2] == 4:
            # 将 BGRA → RGBA，再 HWC → CHW
            sample = image[:, :, [2, 1, 0, 3]].transpose((2, 0, 1))  # 通道排序为 RGBA

        # #infrared
        # # HWC to CHW, 6 channels: [IR_R, IR_G, IR_B, VIS_R, VIS_G, VIS_B]
        # if image.shape[2] == 6:
        #     # OpenCV 读图为BGR，因此先分通道后转换为 RGB 顺序
        #     ir_rgb = image[:, :, :3][:, :, ::-1]  # BGR → RGB
        #     vis_rgb = image[:, :, 3:][:, :, ::-1]
        #
        #     sample = numpy.concatenate([ir_rgb, vis_rgb], axis=2).transpose((2, 0, 1))
        # #END infrared
        else:
            # 原逻辑：RGB三通道
            sample = image[:, :, [2, 1, 0]].transpose((2, 0, 1))
    ...
    def load_image(self, i):
        ...
        # image_vis = cv2.imread(self.filenames[i])  # 可见光 RGB
        # image_ir = cv2.imread(self.filenames_ir[i])  # 红外 RGB
        #
        # if image_vis is None or image_ir is None:
        #     raise FileNotFoundError(f"Image not found: {self.filenames[i]} or {self.filenames_ir[i]}")
        #
        # if image_ir.shape[:2] != image_vis.shape[:2]:
        #     image_ir = cv2.resize(image_ir, (image_vis.shape[1], image_vis.shape[0]))
        #
        # # 拼接为 6 通道图像
        # image = numpy.concatenate([image_ir, image_vis], axis=2)
        # #END infrared
        ...
        if r != 1:
            ...
            if image is None:
                raise FileNotFoundError(f"[❌ ERROR] 图像无法读取: {path}")
    ...
    def load_mosaic(self, index, params):
        ...
        #infrared
        # 先加载一张图，获取通道数
        ref_image, _ = self.load_image(index)
        channels = ref_image.shape[2]

        # 然后根据图像通道数创建 image4
        image4 = numpy.full((self.input_size * 2, self.input_size * 2, channels), 0, dtype=numpy.uint8)
        #END infrared

        # image4 = numpy.full((self.input_size * 2, self.input_size * 2, 3), 0, dtype=numpy.uint8)#visible
    ...
    def load_label(filenames):
        ...
        for filename in filenames:
            try:
                ...
                # verify labels
                # a = f'{os.sep}visible{os.sep}'#visible
                a = f'{os.sep}RGBT{os.sep}'  # infrared
                # a = f'{os.sep}RGBA{os.sep}'  # RGBA
                b = f'{os.sep}labels{os.sep}'
                label_path = filename.replace(a, b)  # 替换路径中的 'images' 为 'labels'
                label_path = os.path.splitext(label_path)[0] + '.txt'  # 修改文件扩展名为 .txt
                # 验证标签文件是否存在
