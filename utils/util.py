...
def compute_metric(output, target, iou_v):
    ...
    n_pred = output.shape[0]
    n_iou = iou_v.shape[0]

    correct = numpy.zeros((n_pred, n_iou), dtype=bool)     # TC
    wrong_cls = numpy.zeros((n_pred, n_iou), dtype=bool)   # FC

    for i in range(n_iou):
        # 所有 IoU >= 阈值的匹配
        y, x = torch.where(iou >= iou_v[i])
        if y.numel():
            ious_ = iou[y, x][:, None]  # 保持 tensor
            matches = torch.cat([torch.stack([y, x], dim=1), ious_], dim=1)  # 拼接后统一再 .cpu().numpy()
            ...
            for m in matches:
                gt_idx = int(m[0])
                pred_idx = int(m[1])
                pred_cls = int(output[pred_idx, 5].item())
                true_cls = int(target[gt_idx, 0].item())
                if pred_cls == true_cls:
                    correct[pred_idx, i] = True
                else:
                    wrong_cls[pred_idx, i] = True

    return (
        torch.tensor(correct, dtype=torch.bool, device=output.device),  # TC
        torch.tensor(wrong_cls, dtype=torch.bool, device=output.device)  # FC
    )

def non_max_suppression(outputs, confidence_threshold=0.3, iou_threshold=0.35):
    max_wh = 7680
    max_det = 300
    max_nms = 30000
    bs = outputs.shape[0]  # batch size
    nc = outputs.shape[1] - 4  # number of 
...
def strip_optimizer(filename):
    x = torch.load(filename, weights_only=False, map_location="cpu")
    print("diaoyongguo")
    ...
    def update(self, model):
        ...
        with torch.no_grad():
            ...
            for k, v in self.ema.state_dict().items():
                if v.dtype.is_floating_point and k in msd and msd[k].shape == v.shape:
                    v *= d
                    v += (1 - d) * msd[k].detach()
                elif v.shape != msd[k].shape:
                    print(f"Skipping parameter '{k}' due to shape mismatch: EMA shape {v.shape}, model shape {msd[k].shape}")
...
class ComputeLoss:
    ...
    def box_decode(self, anchor_points, pred_dist):
        ...
        # Debug: 查看分布范围
        # print("Anchor points range:", anchor_points.min().item(), anchor_points.max().item())
        # print("Predicted lt range:", lt.min().item(), lt.max().item())
        # print("Predicted rb range:", rb.min().item(), rb.max().item())
        # print("Decoded x1y1 range:", x1y1.min().item(), x1y1.max().item())
        # print("Decoded x2y2 range:", x2y2.min().item(), x2y2.max().item())
